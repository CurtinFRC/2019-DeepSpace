...
void HatchIntakeManualStrategy::OnUpdate(double dt) {
  ...
  if (_enabledToggle.Update(_contGroup.GetInput(ControlMap::hatchToggleEnabled))) _enabled = !_enabled;

  if (_enabled) {
    if (_contGroup.GetInput(ControlMap::hatchGrab)) {
      _hatchIntake.SetIntaking();
    } else if (_contGroup.GetInput(ControlMap::hatchRelease)) {
      _hatchIntake.SetOuttaking();
    } else if (_contGroup.GetInput(ControlMap::hatchStow)) {
